...
class MetaTrainer(TrainerBase):
    def __init__(self,
                 ...
                 dataset: Dict[str, Iterable[Instance]],
    ...
    def _get_train_generators(self, num_inner_steps: int):

        num_gpus = len(self._cuda_devices)

        raw_train_generators = {key: self.iterator(train_data, shuffle=self.shuffle)
                                for key, train_data in self.train_data.items()}

        train_generators = {key: lazy_groups_of(lazy_groups_of(raw_train_generator, num_gpus), num_inner_steps)
            for key, raw_train_generator in raw_train_generators.items()}
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for key, train_generator in self.train_generators.items():
            for batch_groups in train_generator:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        self.train_generators = self._get_train_generators(10)
