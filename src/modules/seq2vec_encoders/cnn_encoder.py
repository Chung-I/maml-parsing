...
@Seq2VecEncoder.register("pool-cnn")
class CnnEncoder(Seq2VecEncoder):
    ...
    def __init__(
        ...
        self._activation = conv_layer_activation or Activation.by_name("leaky_relu")()
        self._output_dim = output_dim
        self.pool = (lambda tensor: tensor.mean(dim=2)) if pool_type == 'mean' \
            else (lambda tensor: tensor.max(dim=2)[0])
    ...
    def forward(self, tokens: torch.Tensor, mask: torch.BoolTensor):
        ...
        for i in range(len(self._convolution_layers)):
            ...
            filter_outputs.append(self.pool(self._activation(convolution_layer(tokens))))
        ...
        if self.projection_layer:
            result = self.projection_layer(pooled_output)
        else:
