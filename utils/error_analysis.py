...
def evaluate(gold_ud, system_ud):
    class Score:
        ...
        def __add__(self, other):
            ...
            gold_total = self.gold_total + other.gold_total
            system_total = self.system_total + other.system_total
            assert type(self.aligned_total) == type(other.aligned_total), "aligned_total type not the same"
            aligned_total = self.aligned_total + other.aligned_total if self.aligned_total and other.aligned_total \
                else self.aligned_total
    ...
    metrics = {
        ...
        "FLAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
                                filter_fn=lambda w: not w.is_functional_deprel),
    ...
    for i in range(1, 31):
        metrics[f"LASdep{i}"] = alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
                                                filter_fn=lambda w: abs(int(w.columns[HEAD]) - int(w.columns[ID])) == i)
    LAS_by_sentlen = defaultdict(partial(Score, gold_total=0, system_total=0, correct=0, aligned_total=0))
    bins = [(0, 10), (11, 20), (21, 30), (31, 40), (41, float('inf'))]
    get_bin = construct_bins(bins)
    ptr = 0
    sent_matched_words = []
    sent_gold_words = []
    sent_system_words = []
    for aligned_word in alignment.matched_words:
        sent_span = gold_ud.sentences[ptr]
        if aligned_word.gold_word.span <= sent_span:
            sent_matched_words.append(aligned_word)
            sent_gold_words.append(aligned_word.gold_word)
            sent_system_words.append(aligned_word.system_word)
        else:
            ptr += 1
            sent_alignment = Alignment(sent_gold_words, sent_system_words)
            sent_alignment.matched_words = sent_matched_words
            sent_alignment.matched_words_map = alignment.matched_words_map
            score = alignment_score(sent_alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL]))
            sent_matched_words = []
            sent_gold_words = []
            sent_system_words = []
            sent_len = score.gold_total
            LAS_by_sentlen[get_bin(sent_len)] += score
    for (start, end), item in LAS_by_sentlen.items():
        metrics[f"LASlen{start}{str(end)[:2]}"] = item
...
def main():
    ...
    if not args.verbose and not args.counts:
        ...
        metrics += [f"LASdep{i}" for i in range(1, 31)]
        metrics += list(filter(lambda x: "LASlen" in x, evaluation.keys()))
